...
const createBooking = async (req, res) => {
  try {
    const { propertyId, startDate, endDate, guestName, guestEmail, guestPhone, totalGuests, rooms } = req.body;
    ...
    const totalAmount = (nights > 0 ? nights : 1) * property.price * (rooms || 1);
    
    // Check if admin is booking for a guest (if token provided)
    const isAdminBooking = req.user && req.user.role === 'Admin' && guestName && guestEmail;
    
    // Create booking - Guest bookings are always Pending, Admin bookings are Confirmed
    const booking = new Booking({
      ...
      bookedBy: isAdminBooking ? 'Admin' : 'Guest'
    });
...
const updateBooking = async (req, res) => {
  try {
    const { guestName, guestEmail, guestPhone, totalGuests, rooms, startDate, endDate, status, propertyId } = req.body;
    ...
    if (isPropertyChanging || isDateChanging) {
      ...
      const numRooms = rooms !== undefined ? rooms : booking.rooms || 1;
      booking.totalAmount = (nights > 0 ? nights : 1) * newProperty.price * numRooms;
    ...
    if (rooms !== undefined) booking.rooms = rooms;
